DDBAffiliateNetwork = function()
{
  ...
  this.init = function() {
    ...
    this.tip = d3.tip()
      .attr('class', 'd3-tip')
      .html( function (d) {
        var content = '<h3>'+clearName(d.affiliate_fct)+'</h3><p>'+formatNumber(self.nodeValues.get(d.affiliate_fct_id).affiliate_fct_occurrence)+' entries</p>';
        return content;
      });
    ...
    this.sourceTip = d3.tip().attr('class', 'd3-tip source-tip').html( function (d) { return '<h3>'+d.source.affiliate_fct+'</h3><p>'+formatNumber(self.currentLinks.get(d.source.affiliate_fct_id+"_"+d.target.affiliate_fct_id ).common_occurrence) +' common entries</p>'; } );
    this.targetTip = d3.tip().attr('class', 'd3-tip target-tip').html( function (d) { return '<h3>'+d.target.affiliate_fct+'</h3><p>'+formatNumber(self.currentLinks.get(d.source.affiliate_fct_id+"_"+d.target.affiliate_fct_id ).common_occurrence) +' common entries</p>'; } );
  ...
  this.collectNewNodes = function (e0, e1) {
    ...
    if (this.timelineIDs[0] != e0 || this.timelineIDs[1] != e1) {
      ...
      this.timelineSums.push(this.timeNames[e0].replace("bis", "to").replace("Jahrhundert", "century").replace("vor Christus", "before Christ"));
      this.extractQueryDates(this.timeNames[e0]);

      // gramatically awesome description
      var preposition = (e0>48) ? " from " : " in the ";
  ...
  this.updateAxes = function() {
    ...
    if (this.axes == undefined) {
        ...
        this.axes.append("g")
          ...
          this.axes.select("g.labels").append("text").attr("class", "x").text("Connections").attr("transform", "translate(50," + 200 + ")");
          this.axes.select("g.labels").append("text").attr("class", "y").text("Entries").attr("x",  (this.networkWidth-60)).attr("y", (this.networkHeight-60)).attr("transform", "rotate("+ (this.networkWidth-60)+ " " + (this.networkHeight-140) + "90)");
