...
extractor.personsinEpoch.result = {};
    ...
    //extractor.personsinEpoch.connection.query( 'SELECT affiliate_fct.id AS affiliate_fct_id, affiliate_fct.value AS affiliate_fct, COUNT(DISTINCT affiliate_fct_items.item_id) AS occurrence FROM affiliate_fct_items INNER JOIN affiliate_fct ON affiliate_fct_items.facet_id = affiliate_fct.id INNER JOIN time_fct_items ON time_fct_items.item_id = affiliate_fct_items.item_id WHERE time_fct_items.facet_id = '+ epochID +' GROUP BY affiliate_fct.id ORDER BY occurrence DESC LIMIT ' + limit, 
